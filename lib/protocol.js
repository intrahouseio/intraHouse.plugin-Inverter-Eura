...
function parse(res, waiting) {
  if (!res[0] == ":") throw { message: "Invalid FIRST char!" };

  let adr = Number(res.substr(1, 2));
      ...
      return [
        {
          ...
          value: x1,
          err,
          ext: { status, errstr, volt: x2, current: x3, ratio: x6 }
        },
        { id: "elv_" + adr, value: x2, err },
        { id: "elc_" + adr, value: x3, err }
      ];
    }
...
function getHexAdr(id) {
    ...
    return Number(adr) > 0 ? "0" + adr.toString(16).toUpperCase() : "";
}
